...
def load_k562_mcf7_ocrs():
    ...
    # OCR_FDR_FILE = os.path.join(ROOT_DIR, 'data', 'CTCF_ChIP_seq', 'K562_MCF7_edgeR_dnase_signal_results.csv')
...
def load_diffloop_result():
    ...
    TEST_DIR = os.path.join(ROOT_DIR, 'test', 'dnase_signal')
    ...
    print(len(interactions_with_ocrs)) # 
...
def load_my_result(my_result_file):
    ...
    print(len(interactions_with_genes)) # 43327
    ...
    with open(os.path.join(ROOT_DIR, "test", "dnase_signal", 'my_dci_result', 'my_interactions.csv'), 'w') as f:
        for i in interactions_with_genes:
            f.write(f"{i.chr}\t{i.anchor1.start}\t{i.anchor1.end}\t{i.anchor2.start}\t{i.anchor2.end}\t{i.pvalue}\t{i.fdr}\t{i.gene_pvalues}\t{i.gene_fdrs}\n")
...
def load_diffloop_result_with_counts_after_myQC():
    ...
    with open(os.path.join(ROOT_DIR, "test", "dnase_signal", 'diffloop_interactions_with_counts_after_myQC.csv'), 'w') as f:
        for i in interactions_with_genes:
            f.write(f"{i.chr}\t{i.anchor1.start}\t{i.anchor1.end}\t{i.anchor2.start}\t{i.anchor2.end}\t{i.pvalue}\t{i.fdr}\t{i.gene_pvalues}\t{i.gene_fdrs}\n")

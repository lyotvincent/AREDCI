...
def read_gff3(gff_path, edgeR_rna_file):
    """
    ...
    with open(edgeR_rna_file, 'r') as f:
        lines = f.readlines()
    genes = list()
    for line in lines:
        line = line.strip().split(',')
...
def load_diffloop_result():
    ...
    genes = read_gff3(GFF_PATH, rna_pvalue_file)
    print(genes[0])
    ...
    interactions_with_genes = [interaction for interaction in interactions if interaction.genes]
    print(interactions_with_genes[0])
    print(len(interactions_with_genes)) # 283807
    for i in interactions_with_genes[0].genes:
        print(i)
...
def load_my_result(my_result_file):
    ...
    interactions_with_genes = [interaction for interaction in interactions if interaction.genes]
    print(interactions_with_genes[0])
    print(len(interactions_with_genes)) # 27316
    # for i in interactions_with_genes[0].genes:
    #     print(i)
    for i in interactions_with_genes:
        i.gene_pvalues = ";".join([str(gene.pvalue) for gene in i.genes])
    ...
    with open(os.path.join(TEST_DIR, 'my_dci_result', 'my_interactions.csv'), 'w') as f:
        for i in interactions_with_genes:
            f.write(f"{i.chr}\t{i.anchor1.start}\t{i.anchor1.end}\t{i.anchor2.start}\t{i.anchor2.end}\t{i.pvalue}\t{i.fdr}\t{i.gene_pvalues}\t{i.gene_fdrs}\n")
...
def load_diffloop_result_with_counts_after_myQC():
    ...
    with open(os.path.join(TEST_DIR, 'diffloop_interactions_with_counts_after_myQC.csv'), 'w') as f:
        for i in interactions_with_genes:
            f.write(f"{i.chr}\t{i.anchor1.start}\t{i.anchor1.end}\t{i.anchor2.start}\t{i.anchor2.end}\t{i.pvalue}\t{i.fdr}\t{i.gene_pvalues}\t{i.gene_fdrs}\n")
